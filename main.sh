...
if [ $? -eq 0 ]; then
	...
	echo "Nameserver $nameserversip is not available. Please check and re-run the script!"
	exit
fi
echo
sleep 1
...
# Verify & git clone
if ! (git clone "$client_repo"/"$hostname" > /dev/null 2>&1) then
    echo "Unable to load repository. Please contact Jolera XDR Team."
    exit 1
else
...
if [ \$HEAD1_main_conf = \$HEAD2_main_conf ];then
  ...
  done < <(timeout 20 tail -f /opt/logstash/logs/logstash-plain.log)
fi
sleep 1
